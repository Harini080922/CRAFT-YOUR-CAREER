...
export default class Awards extends Component {
    ...
    updateData = () => {
        ...
        if(this.state.name === '' || this.state.work === '' || this.state.from === ''){
            ...
            mainData.data.awards.push(thisData)
            let x = document.getElementsByClassName("ck-editor__editable");
            let position = mainData.data.awards.length - 1
            mainData.data.awards[position]["about"] = x[0].innerHTML
    ...
    handleDelete = (val) => {
        ...
        let position = this.props.data.awards.length - val - 1
        let x =  this.props.data.awards
        x = x.filter((obj, index) => index!==position)
        
        //update the state
        let result = this.props
        result.data.awards = x
        ...
        return (
            <>
                ...
                <div className="container py-4 my-4  toggle">
                    <h1 className="text-center display-4">AWARDS</h1>
                    <div className="form-row">
                        <div className="col-12 py-2">
                            <label htmlFor={"name"}> Name </label>
                            <input onChange={this.onChangeInput} value={this.state.name} type="text" className="form-control" placeholder="Enter the Competition / Events Name" id={"name"} />
                        </div>
                        ...
                        <div className="col-md-6 col-12 py-2">
                            ...
                            <input onChange={this.onChangeInput} value={this.state.work} type="text" className="form-control" placeholder={"Enter the "+"Awards"+" Name"} id={"work"} />
                        </div>
                        <div className="col-12 py-2">
                            <label > About </label>
                        {/*-----------------------------------------------*/}
                ...
                <hr/>
                    ...
                    <div className="container">
                        {this.props.data.awards.slice(0).reverse().map((obj, index) => {
                            return (
                                <DetailRender 
                                    key={index} 
                                    degree={obj.work} 
